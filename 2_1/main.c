...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i < MAX_LINE_NUMBER; i++){
        ...
        *j = 9; // Starts at 8 because we're ignoring the Game #id: each time
        ...
        bool possible = true; // if the game of the current line is still possible
        ...
        while(!lastWord){
            ...
            if(nbCubes){ // if its a number
                ...
                if (!strcmp(word, "red") && nbCubes > NB_RED) possible = false;
                if (!strcmp(word, "green") && nbCubes > NB_GREEN) possible = false;
                if (!strcmp(word, "blue") && nbCubes > NB_BLUE) possible = false;
            }
            // else printf("Err %d %s ", nbCubes, word);
            if(!possible) break;
        }
        if(possible) sumId+=i+1;
        // printf("%d\n", sumId);
    }
